...
/*
  ...
  printf("adcCount = %3u = 0x%03X, adcVoltage = %.3fV, vBat = %.3f , BatteryLevel = %.3f \n",
             adcCount, adcCount, adcVoltage, vBat , BatteryLevel);
             */
 ...
 // FilteredXangle =  abs (Offset - ((int ((floatValueX * 90) + 1)/5)*5))  ; //rounding to 5
  //Following is Mahony filter for gyro inclusion//
   ...
   // fusion.MahonyUpdate(gx, gy, gz, ax, ay, az, deltat);  //mahony is suggested if there isn't the mag and the mcu is slow
    fusion.MahonyUpdate(xGyro, yGyro, zGyro, floatValueX, floatValueY, floatValueZ, deltat);
